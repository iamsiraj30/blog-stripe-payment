...
export class PostController {
  ...
  @ApiResponse({ status: 403, description: 'Forbidden. Post creation limit reached for subscription plan.' })
  create(
  ...
  @ApiResponse({ status: 200, description: 'List of all active posts retrieved successfully.' })
  findAll() {
  ...
  @ApiResponse({ status: 200, description: 'List of user posts retrieved successfully.' })
  ...
  @ApiResponse({ status: 403, description: 'Forbidden. Only the author can update the post.' })
  @ApiResponse({ status: 404, description: 'Post not found.' })
  update(
    @Param('id') id: string,
    @CurrentUser('id') userId: string,
  ...
  @ApiResponse({ status: 403, description: 'Forbidden. Only the author or an admin can delete.' })
  @ApiResponse({ status: 404, description: 'Post not found.' })
  remove(
    @Param('id') id: string,
    @CurrentUser('id') userId: string,
